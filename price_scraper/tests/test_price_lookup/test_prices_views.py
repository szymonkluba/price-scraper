...
class PricesViewsTests(APITestCase):
    ...
    def setUp(self) -> None:
        ...
        Price.objects.create(price=self.PRODUCT1_PRICE, product=self.product1, store=self.store)
        Price.objects.create(price=self.PRODUCT2_PRICE, product=self.product2, store=self.store)
